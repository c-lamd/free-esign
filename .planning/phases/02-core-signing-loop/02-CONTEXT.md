...
- **Zero-alteration is a hard rule (EXP-02):** overlay signature image objects onto the **original PDF bytes**; never re-encode, reflow, rasterize, or flatten the original content. Success is proven by a **first-512-byte hex comparison** of input vs output content bytes being identical. The exact pdf-lib mechanism (incremental/append save vs standard save with `useObjectStreams`, etc.) is a **research/planning decision** — see Specific Ideas.
