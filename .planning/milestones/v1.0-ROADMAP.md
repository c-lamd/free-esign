...
- [x] **Phase 5: Landing Page + Launch** - Publish the personal hero landing page, pass the privacy audit, deploy to Vercel at free-esign.com (completed 2026-06-17)
...
- [x] 02-03-PLAN.md — Placement + field overlay: makeSimpleViewport, click-to-drop, react-rnd PlacedFieldWidget (drag/resize aspect-lock/select/× + keyboard delete), PDF-space storage (FLD-01, FLD-05, FLD-06, FLD-07)
- [x] 02-04-PLAN.md — Download wiring: TopBar "Download PDF" (disabled at zero fields), exportSignedPdf over current bytes, {name}-signed.pdf, inline export-error banner, image-source export + end-of-phase human-verify (EXP-01, EXP-03)
...
- [x] 03-04-PLAN.md — Undo/redo controls + initials: UndoRedoControls buttons + guarded Cmd/Ctrl+Z/Shift+Z/Ctrl+Y shortcuts, InitialsDrawModal (draw → arm 'initials'), redo-tail + initials-history tests (FLD-02, FLD-09)
...
- [x] 05-01-PLAN.md — Landing slice (Wave 1): 'landing' ViewState as initial view + goToLanding()/startSigning(), src/config.ts BMC constant, LandingPage + Header/Hero/HowItWorks/Privacy/Footer, App.tsx + TopBar wiring, favicon fix + meta, landingPage.test.tsx + documentStore.test.ts update (LND-01, LND-02, LND-03)
- [x] 05-02-PLAN.md — Privacy posture + deploy-readiness (Wave 2): privacyGuard.test.ts source-scan (PRV-03) + vercel.json analytics-key assertion, vercel.json catch-all SPA rewrite, build verification, README deploy runbook, human-verify checkpoint for deferred live deploy/DNS/audit (PRV-03, LND-04)
...
| 5. Landing Page + Launch | 2/2 | Complete   | 2026-06-17 |
